...
<?php startblock('header') ?>
	<title>Create BranchManager - Brac</title>
<?php endblock() ?>
...
<?php 
	...
	*/
	...
	if($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$res = pg_query($db->getRefference(),"SELECT id FROM branch WHERE name ='$name' AND password = '$hashed_password'
			;");
		$row = pg_fetch_row($res);
		$poid = $row['0'];
		//branch_id int,regional_id int
		$query = "CALL addBranch($poid::int,$userid::int);";
