...
<?php 
	$id = $_SESSION['userid'];
	$db = DB::connection();
	$res = pg_query($db->getRefference(),"SELECT name,phone,gender,address FROM po WHERE id=$id;");
	$user = pg_fetch_object($res);

	$poList = [];
	$res = pg_query($db->getRefference(),
		"SELECT poteacher.teacher_id AS id,teacher.name AS Name
		FROM poteacher
		LEFT JOIN teacher ON poteacher.teacher_id = teacher.id 
		WHERE poteacher.po_id = $id;");
 ...
 <div class="d-flex justify-content-center">
	<div class="col-md-6 mt-4">
		<br>
		<h4 class="lead text-center">List of Teacher</h4>
